...
const explorerUrls: Record<number, string> = {
    ...
    31337: 'https://etherscan.io', // localhost/anvil
    11155111: 'https://sepolia.etherscan.io',
};

// Get Tsender contract address for a chain
export function getTsenderAddress(chainId: number): `0x${string}` | undefined {
    const config = chainsToTSender[chainId];
    return config ? (config.tsender as `0x${string}`) : undefined;
}
...
export function isChainSupported(chainId: number): boolean {
    return chainId in chainsToTSender;
}
